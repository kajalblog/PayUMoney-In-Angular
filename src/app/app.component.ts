...
export class AppComponent {
  ...
  payUPayment() {
    ...
    <form action="${info.pay_url}" method="POST" id="payu_form">
       ...
       <input type="hidden" name="productinfo" value="${info.mobile}" />
       <input type="hidden" name="amount" value="${info.amount}" />
       <input type="hidden" name="email" value="${info.email}" />
      <input type="hidden" name="firstname" value="${info.first_name}" />
       <input type="hidden" name="fail_url" value="${info.fail_url}" /> 
        <input type="hidden" name="key" value="${info.payu_merchant_key}" /> 
       <input type="hidden" name="productioninfo" value="${info.plan_name}” /> 
